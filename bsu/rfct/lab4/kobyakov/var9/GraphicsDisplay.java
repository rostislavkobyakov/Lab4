package bsu.rfct.lab4.kobyakov.var9;
...
public class GraphicsDisplay extends JPanel {
    ...
    public GraphicsDisplay() {
        ...
        graphicsStroke = new BasicStroke(2.0f, BasicStroke.CAP_BUTT,
                BasicStroke.JOIN_ROUND, 10.0f, new float[] {40, 10, 10, 10, 10, 10, 20, 10, 20, 10 }, 0.0f);
    ...
    protected void paintMarkers(Graphics2D canvas) {
        ...
        for (Double[] point: graphicsData) {
                ...
                int integerValue = point[1].intValue(); // Получаем целую часть значения функции

            // Проверяем, является ли целая часть квадратом целого числа
            if (PerfectSquare(integerValue)) {
                canvas.setColor(Color.RED); // Установка красного цвета для маркера
            } else {
                canvas.setColor(Color.BLACK);
            }
            ...
            canvas.draw(new Line2D.Double(shiftPoint(center, -2.5, 5), shiftPoint(center, 2.5, 5)));
            canvas.draw(new Line2D.Double(shiftPoint(center, -2.5, -5), shiftPoint(center, 2.5, -5)));
            canvas.draw(new Line2D.Double(shiftPoint(center, 5, -2.5), shiftPoint(center, 5, 2.5)));
            canvas.draw(new Line2D.Double(shiftPoint(center, -5, -2.5), shiftPoint(center, -5, 2.5)));
...
где
    ...
    public boolean PerfectSquare(int integerValue) {
        return Math.sqrt(Math.abs(integerValue)) % 1 ==0;
    }
}
